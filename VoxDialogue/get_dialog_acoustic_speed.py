...
def process_style(style, language='中文'):
    gender, emotion, speaking_rate = [_.strip() for _ in style.strip('() ').split(',')]
    pitch = 'normal'
    if gender == 'female':
        spk_info = language + '女'
    else:
...
if __name__ == '__main__':
    ...
    # 添加命令行参数
    parser.add_argument("--json_dir", type=str, default="examples/paralinguistic_info/speed",
                        help="Path to the input JSON directory.")
    parser.add_argument("--output_dir", type=str, default="path/to/output/speed",
