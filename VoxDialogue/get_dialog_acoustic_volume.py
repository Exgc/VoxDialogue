...
def process_style(style, language='中文'):
    gender, emotion, volume = [_.strip() for _ in style.strip('() ').split(',')]
    ...
    if gender == 'female':
        ...
        spk_info = language + '男'
    insturct_style = f'A {gender} speaker with {pitch} pitch, {speaking_rate} speaking rate, and {emotion} emotion. Volume is {volume}'
...
if __name__ == '__main__':
    ...
    # 添加命令行参数
    parser.add_argument("--json_dir", type=str, default="examples/paralinguistic_info/volume",
                        help="Path to the input JSON directory.")
    parser.add_argument("--output_dir", type=str, default="path/to/output/volume",
