...
import json
from VoxDialogue.tools.asr_zh import pass_or_not
...
def process_style(style, language='中文'):
    gender, emotion, _ = [_.strip() for _ in style.strip('() ').split(',')]
...
def get_dialog_instruct(dialog_dir, language='中文', rank=0, shard=10, generate=True,check=False):
    cosyvoice = CosyVoice('/home/chengxize/project/VoxDialog/pretrained_models/CosyVoice-300M-SFT')
...
if __name__ == '__main__':
    ...
    dialog_json_dir = './Dialogue/scripts/daily_dialogue/speaker_identity/language'
    dialog_dir = '/mnt/disk1/chengxize/data/VoxDialog/speaker_identity/language'
    os.makedirs(dialog_dir, exist_ok=True)
    get_dialog_instruct(dialog_dir=dialog_dir, language='中文', rank=rank, generate=False, shard=2, check=True)
